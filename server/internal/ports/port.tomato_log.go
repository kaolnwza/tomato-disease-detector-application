...
type TomatoLogRepository interface {
	...
	Create(ctx context.Context, logs *model.TomatoLog, farmUUID uuid.UUID, diseaseName string) error
}
...
type TomatoLogService interface {
	...
	Create(ctx context.Context, userUUID uuid.UUID, farmUUID uuid.UUID, desc string, diseaseName string, file multipart.File, bucket string) error
}
