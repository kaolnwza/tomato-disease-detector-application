...
func (r *tomatoLogRepo) Create(
	...
	query := `
		INSERT INTO tomato_log (recorder_uuid, farm_plot_uuid, upload_uuid, tomato_disease_uuid, description
			-- , location
			)
		SELECT 
			$1, 
...
`
	...
	return r.tx.Insert(ctx, query, logs.RecorderUUID.String(), farmUUID.String(), logs.UploadUUID.String(), diseaseName, logs.Description)
}
